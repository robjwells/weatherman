...
on blaze(cityName)
	...
	tell application "TextWrangler"
		tell application "TextWrangler"
			find cityName & "_condition=(.+); \\r" & cityName & "_temperature=(.+);" searching in text 1 of text document 1 options {search mode:grep, starting at top:true, case sensitive:false} without selecting match -- Finds the condition and temperature for cityName
			set theCondition to the grep substitution of "\\1" -- Sets the condition as a variable, ready to pass to InDesign
			set theTemp to ("max " & (grep substitution of "\\2")) -- Sets the temperature as a variable, ready to pass to InDesign
		end tell
	end tell
