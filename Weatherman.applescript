	...
	tell application "TextWrangler"
		tell the front text window
			...
			if (grep substitution of "\\2") is "NA" then -- The only Met Office "weather type" without a number prefix (and so can't be fetched easily from a list)
				if button returned of (display dialog "The Met Office says " & city's name & "'s weather condition is \"not available\"." & return & return & ¬
		...
		tell application "TextWrangler"
			tell the front text window
				...
				if (grep substitution of "\\2") is "NA" then
					if button returned of (display dialog "The Met Office says " & city's name & "'s weather condition is \"not available\" for Sunday." & return & return & ¬
